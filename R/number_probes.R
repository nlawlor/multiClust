...
#' Function to determine the number of gene probes to select for in the gene feature
#' selection process.
...
#' This matrix is an output of the input_file function previously introduced in
#' this package.
#' @param Fixed A positive integer specifying a desired number of gene probes to select for.
#' The default is set to 1000 gene probes.
#' @param Percent A positive integer between 0 and 100 indicating the percentage of total gene
#' probes to select for from the dataset.
#' @param Poly When TRUE, a mean and variance polynomial method is used to determine
#' the number of gene probes to select for. This method uses three second order
#' polynomials to select for the genes with the most variable mean and
#' standard deviations.
#' @param Adaptive When TRUE, Gaussian mixture modeling is used
#' to determine the number of gene probes to select.
#' @return Returns an object with the  number of gene probes that will be selected in
#' the gene feature selection process. If the Adaptive option is chosen,
#' Gaussian mixture modeling files containing information about the data's
...
#' @note This function is not needed to determine the number of gene probes to select
#' for in the Mean_Variance_Polynomial gene selection method. The particular
#' Mean_Variance_Polynomial method does not use a gene probe number input.
...
#'    Percent = 50, Adaptive = NULL)
#' \dontrun{
#' # Example 3: Choosing the Adaptive Gaussian Mixture Modeling method
#' # Very long computation time, so example will not be run
#' gene_num <- number_probes(input = data_file, data.exp = data, Fixed = NULL,
#'    Percent = NULL, Adaptive = TRUE)
#'    }
#' @export
number_probes <- function(input, data.exp, Fixed = 1000, Percent = NULL, Poly = NULL, Adaptive = NULL) {
  # Conditional to cause error if user chose multiple methods at once
  if (is.null(Fixed) == FALSE) {
    if (is.null(Percent) == FALSE) {
      stop("Please choose one of the four options (Fixed, Percent, Poly, Adaptive) only")
    }
  }
  ...
  if (is.null(Fixed) == FALSE) {
    gene_num = Fixed
    # Conditional to produce error if fixed gene number is greater than total genes in matrix
  ...
  # For percent method
  if (is.null(Percent) == FALSE) {
    # Conditional to produce error if Percent chosen is not greater than 0 or less/equal to 1
    if (Percent < 0) {
  ...
  if (is.null(Poly) == FALSE) {
    ...
    print(paste("The polynomial gene probe number is: ", gene_poly, sep = ""))
    return(gene_poly)
  ...
  if (Adaptive == TRUE) {
    ...
    #apply function will apply sd and mean to each value in the vector/data frame
    tmp.mean <- apply(data.min.max, 1, mean)
    tmp.sd <- apply(data.min.max, 1, stats::sd)

    # rnorm generates a random distribution of numbers with a given mean and sd
    ...
    print("Begin applying Gaussian Mixture Modeling to real data, may take several minutes")
    ...
    for (i in 1:dsNoOfRow)
    {
      ...
      cat(rownames(data.min.max)[i], format(gClust$parameters$pro, digits = MAX_NO_OF_DIGITS), file = output, sep="\t")
      cat("\n", file = output)
      close(output)

      output <- file(paste(filePrefix,"output_mean.txt", sep=""), "at")
      cat(rownames(data.min.max)[i], format(gClust$parameters$mean, digits = MAX_NO_OF_DIGITS), file = output, sep="\t")
      cat("\n", file = output)
      close(output)

      output <- file(paste(filePrefix,"output_modelNameG.txt", sep=""), "at")
      cat(rownames(data.min.max)[i], gClust$G, gClust$modelName, file = output, sep="\t")
      cat("\n", file = output)
      close(output)

      output <- file(paste(filePrefix,"output_sigmasq.txt", sep=""), "at")
      cat(rownames(data.min.max)[i], format(gClust$parameters$variance$sigmasq, digits = MAX_NO_OF_DIGITS), file = output, sep="\t")
      cat("\n", file = output)
      close(output)

      output <- file(paste(filePrefix,"output_classification.txt", sep=""), "at")
      cat(rownames(data.min.max)[i], gClust$classification, file = output, sep="\t")
      cat("\n", file = output)
      close(output)
    }
    ...
    print("Begin applying Gaussian Mixture Modeling to Simulated Data, may take several minutes")
    ...
    for (i in 1:dsNoOfRow)
    {
      ...
      output <- file(paste(filePrefixsim,"output_pro.txt", sep=""), "at")
      cat(rownames(data.sim)[i], format(gClust$parameters$pro, digits = MAX_NO_OF_DIGITS), file = output, sep="\t")
      cat("\n", file = output)
      close(output)

      output <- file(paste(filePrefixsim,"output_mean.txt", sep=""), "at")
      cat(rownames(data.sim)[i], format(gClust$parameters$mean, digits = MAX_NO_OF_DIGITS), file = output, sep="\t")
      cat("\n", file = output)
      close(output)

      output <- file(paste(filePrefixsim,"output_modelNameG.txt", sep=""), "at")
      cat(rownames(data.sim)[i], gClust$G, gClust$modelName, file = output, sep="\t")
      cat("\n", file = output)
      close(output)

      output <- file(paste(filePrefixsim,"output_sigmasq.txt", sep=""), "at")
      cat(rownames(data.sim)[i], format(gClust$parameters$variance$sigmasq, digits = MAX_NO_OF_DIGITS), file = output, sep="\t")
      cat("\n", file = output)
      close(output)

      output <- file(paste(filePrefixsim,"output_classification.txt", sep=""), "at")
      cat(rownames(data.sim)[i], gClust$classification, file = output, sep="\t")
      cat("\n", file = output)
      close(output)
    }

    print("Gaussian Mixture Modeling has been applied to your simulated data")
    ...
    {
      mat[rank(as.numeric(mat[,n]), ties.method ="random"),] <- mat[c(1:nrow(mat)),]
      return(mat)
    }

    # Function to compute the TRank value
    compute.TRank <- function(fn.dataModelNameG, fn.dataClassification,fn.dataSigmasq,fn.dataMean,fn.dataPro,curr.Sigmasq)
    {
      # Model Name and G
      dataModelNameG <- utils::read.delim2(fn.dataModelNameG, header = FALSE, row.names = 1, sep = "\t", quote="\"", dec=".", fill=TRUE)

      # discrete classification produced by mclust
      dataClassification <- utils::read.delim2(fn.dataClassification, header = FALSE, row.names = 1, sep = "\t", quote="\"", dec=".", fill=TRUE)

      # Extract Properties of the input file
      noOfGene = NROW(dataModelNameG) # number of Gene
      maxG <-max(dataModelNameG[,1]) # maximum number of components.
      noOfSample = NCOL(dataClassification) #number of samples (260, vary from dataset to dataset)

      # sigma square produced by mclust
      dataSigmasq <- utils::read.delim2(fn.dataSigmasq, header = FALSE, row.names = 1, col.names=c(1:(maxG+1)), sep = "\t", quote="\"", dec=".", fill=TRUE)

      # mean produced by mclust
      dataMean <- utils::read.delim2(fn.dataMean, header = FALSE, row.names = 1, col.names=c(1:(maxG+1)), sep = "\t", quote="\"", dec=".", fill=TRUE)

      # pi-zero produced by mclust
      dataPro <- utils::read.delim2(fn.dataPro, header = FALSE, row.names = 1, col.names=c(1:(maxG+1)), sep = "\t", quote="\"", dec=".", fill=TRUE)

      # discrete classification produced by mclust
      dataClassification <- utils::read.delim2(fn.dataClassification, header = FALSE, row.names = 1, col.names=c(1:(noOfSample+1)), sep = "\t", quote="\"", dec=".", fill=TRUE)
      ...
      for (i in 1:noOfGene)
      {
        ...
        # Get the model information. Extract the number of clusters and model name for each gene
        ...
        tmpPWT <- matrix(data=0, nrow=tmpG, ncol=tmpG) #pair-wised, use zero as default as later on summation is used.
        ...
        for(j in 1:(tmpG-1))
        {
          ...
          else # 2-GMM onwards
          {
            for(k in (j+1):(tmpG-1))
            {
              if(k>j) # added on 15 July 2011, otherwise wrong.
              {
                if(tmpModelName=="E")
                {
                  tmpPWT[j,k] = (abs(dataMean[tmpMMADBID,j]- dataMean[tmpMMADBID,k])/sqrt(2*dataSigmasq[tmpMMADBID,1]+curr.Sigmasq))*((dataPro[tmpMMADBID,j]*dataPro[tmpMMADBID,k])/((dataPro[tmpMMADBID,j]+dataPro[tmpMMADBID,k])^2))
                }
                else if(tmpModelName=="V")
                {
                  tmpPWT[j,k] = (abs(dataMean[tmpMMADBID,j]- dataMean[tmpMMADBID,k])/sqrt(dataSigmasq[tmpMMADBID,j]+dataSigmasq[tmpMMADBID,k]+curr.Sigmasq))*((dataPro[tmpMMADBID,j]*dataPro[tmpMMADBID,k])/((dataPro[tmpMMADBID,j]+dataPro[tmpMMADBID,k])^2))
                }
              }
            }
          }
    ...
    tmp.fn.dataModelNameG = paste(filePrefixsim, "output_modelNameG.txt", sep = "")
    tmp.fn.dataClassification = paste(filePrefixsim,"output_classification.txt", sep = "")
    tmp.fn.dataSigmasq = paste(filePrefixsim,"output_sigmasq.txt", sep = "")
    tmp.fn.dataMean = paste(filePrefixsim,"output_mean.txt", sep = "")
    tmp.fn.dataPro = paste(filePrefixsim,"output_pro.txt", sep = "")
    ...
    x<-compute.TRank(tmp.fn.dataModelNameG, tmp.fn.dataClassification,tmp.fn.dataSigmasq,tmp.fn.dataMean,tmp.fn.dataPro,tmp.Sigmasq)
    rownames(x)=as.character(t(x[,1]))

    # Begin reading in real data files for TRank calculation
    tmp.fn.dataModelNameG = paste(filePrefix, "output_modelNameG.txt", sep = "")
    tmp.fn.dataClassification = paste(filePrefix,"output_classification.txt", sep = "")
    tmp.fn.dataSigmasq = paste(filePrefix,"output_sigmasq.txt", sep = "")
    tmp.fn.dataMean = paste(filePrefix,"output_mean.txt", sep = "")
    ...
    y<-compute.TRank(tmp.fn.dataModelNameG, tmp.fn.dataClassification,tmp.fn.dataSigmasq,tmp.fn.dataMean,tmp.fn.dataPro,tmp.Sigmasq)
    ...
    # Choose the TRank with a FDR value of 0.01 for the lowest number of selected genes
    # a represents the index of the TRank value to be chosen
    FDRvalue = 0.01
    index = which(FDR[,4] >= FDRvalue)

    # Conditional if FDR value never reaches specified value (eg. 0.01)
    # Reduce the FDRvalue
    if (length(index) == 0) {
    ...
    # Conditionals to make sure only one TRank value is chosen in given parameters
    if (length(TRank) == 0) {
      TRank <- 0.30
      print("Arbitrary TRank value of 0.30 chosen")
      print("The specified FDR cutoff did not occur in the data")
    }
    # Conditional if more than 1 TRank value exists with the same FDR cutoff
    if (length(TRank) > 1) {
      TRank <- min(TRank)
    }
