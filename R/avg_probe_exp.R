...
#' Function to produce a matrix containing the average expression of each gene probe
#' within each sample cluster.
...
#' @return Returns an object matrix with the average mean expression for each probe
#' in each sample cluster. Also outputs the object to a text file.
#' @examples
...
avg_probe_exp <- function(sel.exp, samp_cluster, data_name, cluster_type = "HClust",
  ...
  for (i in 1:numofclust) {
    ...
    # savednames contains the samples that belong to a given cluster number
  ...
  print("Your matrix containing the average gene probe expression in each cluster is finished")
  return(meansofclust)
}
